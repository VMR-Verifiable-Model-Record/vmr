//! `vmr trust-store add` (Phase 5 task 5.6b, docs/dev/phase5.md C8).
// ============================================================================
...
use crate::cli::{AttestationArg, TrustStoreAddArgs};
...
pub fn add(args: &TrustStoreAddArgs) -> Result<Output, CliError> {
    ...
    let (mut document, created) = match std::fs::metadata(&args.trust_store) {
        Ok(_) => (load_trust_store(&args.trust_store)?.to_document(), false),
        ...
        Err(e) => {
            return Err(CliError::input(format!("cannot read trust store {}: {e}", shown(&args.trust_store))))
        }
    };
